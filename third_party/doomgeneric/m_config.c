...
char *M_GetSaveGameDir(char *iwadname)
{
    ...
    else
    {
        ...
        free(topdir);
#else
        /* NaumiOS port: was ".savegame/" — a leading dot has no meaning in
           FAT 8.3 names (unlike Unix dotfile convention), and this
           project's FAT16 writer doesn't special-case it, so the on-disk
           short name it would end up creating doesn't round-trip back to
           the same string on lookup. A plain 8.3-safe name sidesteps the
           problem entirely rather than teaching the filesystem code a
           workaround for a convention it was never going to need again. */
